...
/*------------------------ Αυτή η class υλοποιεί ένα τραπέζι για Card_7_Stud-------------------------*/
public class Card_7_Stud_Table extends Table 
{
	/**
	...
	private int ID;
	/**
	 * @uml.property  name="cards_dealed"
	 */
	private int cards_dealed;
	
	Card_7_Stud_Table(int players_limit, Beting_System B, boolean G)// Constructor
	{
	...
	public void Add_Player(boolean automatic)
	{
		...
		if (Get_Players() < Get_Players_Limit())// Aν υπάρχει χώρος για άλλον παίχτη
			Players.add(new Card_7_Stud_Player(automatic));
		else// Αλλιώς
	...
	public void Proceed_dealing()
	{
		...
		if (cards_dealed >= 3 && cards_dealed < 7)//
		{
			for (int i=0;i<Get_Players();i++)
			{
				Players.get(i).Set_Max_Cards(Players.get(i).Get_Max_Cards() + 1);
			}
			Deal_Cards();
			cards_dealed++;
		}
